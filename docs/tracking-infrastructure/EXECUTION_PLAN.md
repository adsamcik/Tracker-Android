...
The current 61-table `28.json` is therefore not frozen. Host compilation is useful, but only the
populated Android migration/open/reopen/product test can close TI-184.
...
| TI-184 | TI-D068; migration safety | A/G | `AppDatabaseMigration27To28Test`, fixture assets, production query/export/delete/import paths | TI-180–TI-183 | Populated v27 opens, migrates, closes, reopens, queries, exports, deletes, and recovers without fabricated authority or lost facts; checksum-valid pending v27 WAL drains once only through its released legacy projection contract; interrupted automatic state is inert/finalized before any active read | populated migrate/reopen/production-DAO/delete/reopen passes on `Medium_Phone`; full Tracker database/backup selection now fails before merge mutation or false completion; cold empty-target restore, portable export/provenance-fenced merge, and legacy-WAL drain remain | Backup/recovery marker required; migration failure must preserve recoverable v27 database | `IN_PROGRESS` |
...
| TI-311 | TI-D055/TI-D056; purpose separation | B/D | source delivery models, WAL admission, source-specific batch splitters | TI-181, TI-182 | Provider observations and operational request outcomes are distinct; attempts/ticks never qualify `RECORDING`; every admitted unit is authorization-homogeneous; control evidence is bounded and absent from product/export | six boundary-crossing delayed-batch tests; control-only history/export assertions | Behind per-source writer flag; rollback retains bounded operational evidence | `IN_PROGRESS` |
...
| TI-413 | Source contract: Activity | B/D/E | shared transition/activity runtime and typed movement bands | shared minimum substrate, Activity-relevant TI-211/TI-212 | Transition control is purpose-limited; capture is separate; fresh STILL semantics hold; sampled mode used only for direct quality need; stale automation cannot start | source, automation epoch, process/reboot, product query tests | `writer_v2:activity`; control remains independently revocable | `BLOCKED` |
...
3. Preserve the now-passing populated v27→v28 fixture on `Medium_Phone`: all six typed source
   ...
   fail-closed, and production deletion/reopen proves no Location-child resurrection. Extend TI-184
   separately with portable export/import and backup-recovery evidence; do not fold those paths into
   schema migration code.
4. Adopt the now-tested atomic delivery-admission seam source by source. It persists a compact
   process-stable source-native identity, keeps original batch indexes, resolves each unit against
   observed-time authorization, and commits its sequence range and WAL rows atomically. Activity is
   the first adapter candidate; the remaining five splitters, source cursors/gaps, scoped deletion,
   retained-floor no-resurrection, and the released v27-WAL drain remain separate gates.
...
7. Implement the smallest end-to-end vertical, Steps, with its own writer/cursor/correction,
   ...
   otherwise-ready independent source.
