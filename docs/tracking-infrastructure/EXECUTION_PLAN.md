...
| TI-183 | Final plan §8–9; one writer, no resurrection | D | source cursors/gaps, deletion fences, destination owner fence | TI-180 | One cursor/gap lane per source; scoped source/purpose/time deletion fences replay and import; one persisted destination owner generation fences both legacy and target writes | Steps exact owner checks, immutable service-run history selection, atomic activation/rollback, and ABA-safe empty-generation re-arm pass at `9b8ab4b43`; scoped delete→replay/import, the production deletion-service seam, query continuity, and other source lanes remain | Per-source owner flag; rollback changes owner only after contain/drain/catch-up | `IN_PROGRESS` |
