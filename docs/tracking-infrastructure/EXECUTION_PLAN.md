...
- The corrected R1 review authorized TI-410's dormant source-local manual/session Steps writer,
  ...
  entry grouping, source materialization, typed export/import, selected deletion, device proof, and
  an explicit release action remain blocked. Test correction `d067704a9` and the passing full host
  aggregate preserve the no-reclamation boundary. Other materializers, automatic/ambient Steps, broad
...
1. The contained Steps transition is `IN_REVIEW`, not activated. Its full
   ...
   test and exact run-to-presentation settlement now pass. Preserve TI-410's remaining product
   gates: new-v28 reverse binding, logical-entry grouping, ordinary source-only discovery, truthful
   baseline/covered-zero/positive/partial states, one production session/Today query and every
   applicable numeric consumer, source-aware tracking UI, centralized typed manual start, and typed portable Steps export/import.
...
7. Continue manual/session Steps from the existing source-local receipt/fact/coverage/owner records,
   ...
   settlement. Add the new-v28 Steps reverse-binding check, logical-entry grouping, source-aware
   discovery, typed Step boundaries/membership, portable export/import, selected deletion, and one
