...
- The corrected R1 review authorizes TI-410's dormant source-local manual/session Steps writer,
  durable identity bridge, and exact destination fence. Production query/UI wiring now waits for
  segment-effective writer provenance, service-run completeness, cutover/rollback, retention, and
  deletion gates under TI-D103. Other materializers, automatic/ambient Steps, broad history UI, and
  every other source remain blocked by their independent gates.
...
| ST-02a manual/session Steps | Lead orchestrator with non-overlapping focused owners | Steps runtime, source-local cursor/facts/correction/deletion/export/query, existing product/start adapters | in progress; dormant writer/retention sub-slice committed in `8bb6d606f` and `af846143c` | manual only-Steps reaches `RECORDING`, `MATERIALIZED`, and a production query with no Activity/Location demand and no generic platform; every existing manual-start surface, completeness-sensitive consumer, and typed portable round trip agrees |
...
The current schema contains 51 released-v27 tables and exactly 15 narrowly owned v28 additions. This constrains
TI-180 as follows:
...
The current 66-table `28.json` is therefore not frozen. Two additions are the narrowly scoped
released-v27 recovery obligation/target tables, two are the Activity automatic-action/epoch
records, and one is the source-local product-lane activation/cursor/retention fence required by
R1. They do not recreate the removed generic Phase 3 platform. Populated Android
...
| TI-183 | Final plan §8–9; one writer, no resurrection | D | source cursors/gaps, deletion fences, destination owner fence | TI-180 | One cursor/gap lane per source; scoped source/purpose/time deletion fences replay and import; one persisted destination owner generation fences both legacy and target writes | Steps exact owner checks and ABA-safe CAS pass at `ef1da62d2`; mixed-batch cutover, segment-effective history, delete→replay/import, and rollback-generation tests remain | Per-source owner flag; rollback changes owner only after drain/catch-up | `IN_PROGRESS` |
...
| TI-323 | One writer; Location protection | D/G | rollout owner state, legacy and target destination gateways | TI-183 and an actual candidate writer for the source | Owner generation is rechecked inside destination transaction; cutover waits for cursor catch-up and legacy drain; rollback keeps all facts queryable; Location legacy owner remains until shadow decision | Steps now proves exact legacy/candidate checks only; segment-effective history, latched mixed-destination commit, cutover/crash/delete-recovery/rollback tests remain | `destination_owner:<source>`; data-preserving rollback | `IN_PROGRESS` |
| TI-410 | Source contract: Steps manual/session vertical | B/D/E | Steps runtime, typed boundaries/intervals, Steps-local writer/cursor/delete/export/query adapter | corrected R1 activation/recovery boundary, relevant TI-180–TI-183/TI-312 substrate | App-scoped counter; durable baseline; baseline/covered-zero/positive/partial distinctions; positive post-baseline delta is `RECORDING`; session-local correction, no-resurrection, export and one production query work while every unrelated projector is absent | dormant writer replay/poison/retention passes at `8bb6d606f` + `af846143c`; `ef1da62d2` adds paired session/run identity, zero-covered segment retention, exact destination fence, migration/device proof. Segment-effective provenance, per-run completeness, safe cutover/rollback/delete, truthful observable query, correction/retraction, typed export/import, process/reset/device and product consumers remain | `writer_v2:steps`, `history_v2:steps`; no executable binding exists yet | `IN_PROGRESS` |
| TI-410B | Source contract: Steps automatic mode | B/C/D/E | TI-410 Steps lane plus declared Activity control, automation lifecycle, bounded control evidence | TI-410; explicit step-corroboration decision; legal fresh Activity trigger; purpose-limited control retention/no-export proof | Only Steps is captured; Activity is `CONTROL` only; fresh control starts the logical session; positive Steps delta records; control-only observations never enter history/export; contained optional control is terminal rather than retry work | automatic only-Steps integration, process/reboot/stale-trigger, control-retention/export, production query assertions | `automatic_v2:steps`; rollback disables automatic demand without disabling manual Steps | `BLOCKED` |
...
1. The three fresh corrected-boundary R1 adversaries passed the shared boundary only for beginning
   TI-410. Preserve their completion gates: exactly one executable Steps lane; a single atomic
   fact/receipt/cursor/deletion transaction; truthful baseline/covered-zero/positive/partial states;
   one production session/Today query; existing source-aware tracking UI; centralized typed manual
   start; and typed portable Steps export/import. Do not broaden this into another platform.
...
7. Implement manual/session Steps now with the minimum source-local receipt/fact/coverage/owner
   records justified by current schema evidence, one source cursor/drain, narrow scoped deletion and
   destination-owner fences, typed Step boundaries/membership, portable export/import, and one
   production session/Today query. Gate automatic Steps separately on declared
