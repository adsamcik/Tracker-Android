...
class StorageAnalyzerTest {
	...
	private fun mockDb(
		...
		every { tableCursor.moveToNext() } answers {
			if (callCount < tables.size) { callCount++; true } else { false }
		}
		var tableIdx = -1
		every { tableCursor.getString(0) } answers { tableIdx++; tables[tableIdx] }
		every { tableCursor.close() } returns Unit
		every { db.query(match<String> { it.startsWith("SELECT name FROM sqlite_master") }) } returns tableCursor
	...
	inner class DailyGrowth {
		...
		@Test
		fun dailyGrowthIsPositiveWhenDataExists() {
			// Use a far-past timestamp so the test is stable regardless of when it runs
			val oldestMs = 1_000_000_000_000L // ~2001, always far in the past
			val result = StorageAnalyzer(mockDb(
				pageSize = 4096,
				pageCount = 1000,
				tables = listOf("location_sample"),
				rowCounts = mapOf("location_sample" to 100L),
				minTimes = mapOf("location_sample" to oldestMs),
			)).analyze()
			assertTrue(result.estimatedDailyGrowthBytes > 0,
				"Daily growth should be positive when data exists")
		}

		@Test
		fun dailyGrowthIsZeroWhenOldestIsNull() {
			val result = StorageAnalyzer(mockDb(
				tables = listOf("table1"),
				rowCounts = mapOf("table1" to 10L),
				minTimes = mapOf("table1" to null),
			)).analyze()
			assertEquals(0L, result.estimatedDailyGrowthBytes)
		}
	}
}
