...
sealed interface LegacyDatabaseStartupResult {
	...
	data class ActiveDatabaseRetryable(
...
class LegacyDatabaseUpgradeCoordinator @Inject constructor(
	...
	suspend fun ensureReady(retry: Boolean = false): LegacyDatabaseStartupResult = mutex.withLock {
		...
		return@withLock try {
			...
			readyForThisProcess.set(false)
			Tracebox.log.warn(
				TrackerTraceboxTemplates.ACTIVE_DATABASE_OPEN_RETRYABLE,
				public(retryable.reason.failureCode),
			)
			LegacyDatabaseStartupResult.ActiveDatabaseRetryable(retryable.reason)
