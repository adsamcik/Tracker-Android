 ...
 * writing 310 individual placeholder strings ("Distinct Cells 10" / "Progress
 * achievement for distinct cells threshold 10."), titles and descriptions are
...
internal object AchievementFormatting {
	...
	fun rememberDescription(definition: AchievementDefinition): String {
		val context = LocalContext.current
		return remember(definition.id, context) {
			formatDescription(context, definition)
		}
	}
	...
	fun rememberValueLabel(current: Double, definition: AchievementDefinition): String {
		...
		return remember(definition.id, current, context) {
			formatValueLabel(context, current, definition)
		}
	}

	private fun formatTitle(context: Context, def: AchievementDefinition): String {
	...
	private fun formatDescription(context: Context, def: AchievementDefinition): String {
		val res = context.resources
		return when (def.metric) {
	...
	private fun formatValueLabel(context: Context, current: Double, def: AchievementDefinition): String {
		val res = context.resources
