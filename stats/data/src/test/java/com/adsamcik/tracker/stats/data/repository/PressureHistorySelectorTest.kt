...
class PressureHistorySelectorTest {
	...
	private suspend fun insertFixture(
		...
		database.sourceSessionDao().saveCompleteness(
			SourceSessionCompletenessEntity(
				...
				sourceInstanceId = "pressure-provider-1",
				registrationGeneration = 1L,
				lastAdmissionOrdinal = factAdmissionOrdinal,
				lastSourceSequence = factAdmissionOrdinal,
	...
	private fun serviceRun() = SourceServiceRunEntity(
		...
		startedElapsedNanos = RUN_START_ELAPSED_NANOS,
		...
		startOrigin = MANUAL_START_ORIGIN,
		runRevision = 1L,
		startDeliveryToken = "delivery-$RUN_ID",
	...
	private fun pressureBinding() = SessionManifestSourceEntity(
		logicalTrackingId = LOGICAL_ID,
		manifestRevision = 1L,
		sourceKind = SourceDestinationOwnerEntity.SOURCE_PRESSURE,
	...
	private fun pressureFact(
		...
		val unsigned = PressureFactRevisionEntity(
			...
			intervalStartTimeMs = PRESSURE_START_MS,
			intervalEndTimeMs = PRESSURE_END_MS,
			windowStartElapsedRealtimeNanos = PRESSURE_START_ELAPSED_NANOS,
			windowEndElapsedRealtimeNanos = PRESSURE_END_ELAPSED_NANOS,
