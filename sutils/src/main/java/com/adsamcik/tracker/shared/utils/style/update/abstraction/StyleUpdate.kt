...
internal abstract class StyleUpdate {
	...
	fun onEnable(context: Context, configData: StyleConfigData) {
		if (isEnabled) ReporterFacade.report("Style update is already in enabled state.")
	...
	fun onDisable(context: Context) {
		if (!isEnabled) ReporterFacade.report("Style update is already in disabled state.")
