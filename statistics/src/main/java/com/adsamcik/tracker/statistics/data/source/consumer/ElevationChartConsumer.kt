...
class ElevationChartConsumer : StatDataConsumer {
	...
	override fun getData(context: Context, data: StatDataMap): Any {
		...
		return altitudeList.map { ChartPoint(it.x.toFloat(), it.y.toFloat()) }
	}
	...
	override fun serializeData(data: Any, moshi: Moshi): String {
		val arrayType = Types.newParameterizedType(List::class.java, ChartPoint::class.java)
		@Suppress("UNCHECKED_CAST")
		return moshi.adapter<List<ChartPoint>>(arrayType).toJson(data as List<ChartPoint>)
	}

	override fun deserializeData(data: String, moshi: Moshi): Any {
		val arrayType = Types.newParameterizedType(List::class.java, ChartPoint::class.java)
		@Suppress("UNCHECKED_CAST")
		return moshi.adapter<List<ChartPoint>>(arrayType).fromJson(data) as Any
	}
