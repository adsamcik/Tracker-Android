...
class OptimizedLocationDataProducer : StatDataProducer {
	...
	override fun produce(rawDataMap: RawDataMap, dataMap: StatDataMap): Any {
		val locations = rawDataMap.requireData<List<LocationSample>>(StatDataSource.LOCATION)

		val mappedLocations = locations.mapNotNull { sample ->
		...
		if (mappedLocations.size <= 2) return mappedLocations
