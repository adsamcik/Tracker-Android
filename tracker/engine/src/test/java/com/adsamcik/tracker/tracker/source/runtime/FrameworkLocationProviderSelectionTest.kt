...
class FrameworkLocationProviderSelectionTest {
	...
	fun `balanced tracking uses GPS when the network provider is unavailable`() {
		...
		assertEquals(LocationManager.GPS_PROVIDER, LocationMode.BALANCED.toFrameworkProvider(manager))
	}
	...
	fun `high accuracy uses network when GPS is unavailable`() {
		...
		assertEquals(LocationManager.NETWORK_PROVIDER, LocationMode.HIGH_ACCURACY.toFrameworkProvider(manager))
	}
	...
	fun `active tracking uses passive only when no active provider is available`() {
		...
		assertEquals(LocationManager.PASSIVE_PROVIDER, LocationMode.BALANCED.toFrameworkProvider(manager))
		assertEquals(LocationManager.PASSIVE_PROVIDER, LocationMode.HIGH_ACCURACY.toFrameworkProvider(manager))
	}
}
