...
| `testReleaseEvidence` | Runs the release validator's controlled bad-input Python fixtures. |
| `verifyReleaseDependencyMetadata` | Verifies that selective dependency checksums match the canonical release-input policy. |
...
Detekt uses the checked-in `detekt.yml` and `detekt-baseline.xml`. The reviewed baseline records 6,347
inherited finding IDs. This is the exact set union of the two `dev/v10` parent ledgers: 5,144 remote
findings and 6,137 local-parent findings, with 4,934 shared IDs. Detekt configuration validation and
warnings-as-errors are enabled, and an unbaselined finding fails `detekt`.
...
| `build_validation` | `Validate release bundle evidence` | Builds and verifies the release AAB and representative bundletool APK evidence. | `release-evidence-<commit>`: release evidence, AAB, R8 mapping, and native symbols; retained 7 days. |
...
`**/build/reports/lint-results-*.*`, and `build/reports/detekt/**`. The exact release-evidence paths
are `build/release-evidence/**`, `app/build/outputs/bundle/release/*.aab`,
`app/build/outputs/mapping/release/**`, and `app/build/outputs/native-debug-symbols/release/**`.
