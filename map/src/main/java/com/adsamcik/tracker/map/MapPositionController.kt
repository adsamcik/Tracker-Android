...
class MapPositionController(context: Context, map: GoogleMap) {
	...
	fun onNewPosition(latlng: LatLng, accuracy: Float) {
		...
		userCenter.isVisible = true

		userDirection.position = latlng
		userDirection.isVisible = hasValidDirection

		userActivity.position = latlng
	...
	fun onNewActivity(groupedActivity: GroupedActivity) {
		if (groupedActivity == GroupedActivity.UNKNOWN) {
			...
			userActivity.setIcon(activityIconList[groupedActivity])
			userActivity.isVisible = hasLocation
		}
	}
	...
	fun onDirectionChanged(radians: Double) {
		...
		userDirection.rotation = radians.toDegrees().toFloat()
	}
}
