		...
		is HistoryCapture.Exact -> {
			...
			when {
				sourceSets.all { it == setOf(HistorySource.WIFI) } -> when (
					...
					is WifiHistoryQuery.Found -> TripDetailSourcePresentation.WifiOnly(query.entry)
				...
				sourceSets.all { it == setOf(HistorySource.CELL) } -> when (
					...
					is CellHistoryQuery.Found -> TripDetailSourcePresentation.CellOnly(query.entry)
					null,
				...
				sourceSets.all { it == setOf(HistorySource.PRESSURE) } -> {
					...
					TripDetailSourcePresentation.PressureOnly(pressureHistory.history.pressure)
				}
