...
class StatsLoadStateTest {
	...
	inner class DayBarTests {
		...
		fun `default sessionCount is 0`() {
			val bar = DayBar(dayLabel = "Tue", distanceM = 0f, epochDay = 0L)
			bar.sessionCount shouldBe 0
		}

		@Test
		fun `equality works`() {
			val b1 = DayBar(dayLabel = "Wed", distanceM = 100f, epochDay = 1L)
			val b2 = DayBar(dayLabel = "Wed", distanceM = 100f, epochDay = 1L)
			b1 shouldBe b2
		...
		fun `inequality for different values`() {
			val b1 = DayBar(dayLabel = "Wed", distanceM = 100f, epochDay = 1L)
			val b2 = DayBar(dayLabel = "Thu", distanceM = 100f, epochDay = 2L)
			b1 shouldNotBe b2
		}

		@Test
		fun `copy works`() {
			val original = DayBar(dayLabel = "Fri", distanceM = 500f, epochDay = 5L)
		...
		fun `destructuring works`() {
			val bar = DayBar(dayLabel = "Sat", distanceM = 750f, epochDay = 6L, sessionCount = 3)
			val (dayLabel, distanceM, epochDay) = bar
			dayLabel shouldBe "Sat"
			distanceM shouldBe 750f
			epochDay shouldBe 6L
			bar.sessionCount shouldBe 3
		}
	}
}
