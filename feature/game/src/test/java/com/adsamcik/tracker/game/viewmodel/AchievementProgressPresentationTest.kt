...
class AchievementProgressPresentationTest {
	@Test
	fun `qualified progress is visible without fabricating a recent unlock time`() {
		val product = requireNotNull(qualified(AchievementProgressEntity.AUTHORITY_STATE_READY)
			.toProductProgress())

		product.metric shouldBe MetricKey.GOAL_STREAK_DAYS
	...
	fun `unavailable source metrics are absent from list and detail composition`() {
		qualified(AchievementProgressEntity.AUTHORITY_STATE_MATERIALIZING)
	...
	private fun qualified(state: String) = AchievementProgressEntity(
