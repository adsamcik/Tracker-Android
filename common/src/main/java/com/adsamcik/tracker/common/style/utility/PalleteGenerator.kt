...
class PalleteGenerator {
	...
	fun generate(
			colorsCount: Int = 8,
			checkColor: (Int) -> Boolean = { true },
			forceMode: Boolean = false,
			quality: Int = 50,
			ultra_precision: Boolean = false,
		...
		if (forceMode) {
			...
			var kMeans = mutableListOf<DoubleArray>()
			for (i in 0 until colorsCount) {
...
//&& checkColor2([candidateKMean[0], candidateKMean[1], candidateKMean[2]]) && candidateKMean
	...
	fun getColorDistance(
		...
		return when (type) {
			...
			DistanceType.Euclidian -> euclideanDistance(lab1, lab2)
	...
	val simulateCache = mutableMapOf<String, DoubleArray>()
	...
	fun simulate(lab: DoubleArray, type: DistanceType, amount: Int = 1): DoubleArray {
		...
		val chroma_x = x / (x + y + z);
		val chroma_y = y / (x + y + z);
		// Generate the "Confusion Line" between the source color and the Confusion Point
		val m = (chroma_y - confuseY) / (chroma_x - confuseX); // slope of Confusion Line
		val yint = chroma_y - chroma_x * m; // y-intercept of confusion line (x-intercept = 0.0)
		// How far the xy coords deviate from the simulation
		val deviate_x = (confuseYint - yint) / (m - confuseM);
		val deviate_y = (m * deviate_x) + yint;
		// Compute the simulated color"s XYZ coords
		val dX = deviate_x * y / deviate_y;
		val dZ = (1.0 - (deviate_x + deviate_y)) * y / deviate_y;
		// Neutral grey calculated from luminance (in D65)
		val neutral_X = 0.312713 * y / 0.329016;
		val neutral_Z = 0.358271 * y / 0.329016;
		// Difference between simulated color and neutral grey
		val diffX = neutral_X - dX;
		val diffZ = neutral_Z - dZ;
